...
The [0.3.2 release workflow](https://github.com/calvinh99/ScholarsEye/actions/runs/35495766024) passed. Both the latest feed and exact released ZIP were downloaded using `curl -q` without authorization, cookies, or user curl configuration. The feed identified build 5 and its public versioned ZIP URL. The archive's Ed25519 signature and the extracted app's complete code signature passed verification; its bundle contained no private-repository identifier. Computer use launched that exact CI-produced app and checked the live signed feed, which reported **ScholarsEye 0.3.2 is currently the newest version available**, without a GitHub connection step.
